...
namespace PhoneBookServer
{
    ...
    class PhoneBookService : IPhoneBookService
    {
        ...
        public bool UpdatePerson(ref PhoneBookRecord recordToUpdate)
        {
            if(recordToUpdate == null)
            {
                //throw new ArgumentNullException("recordToUpdate");
                recordToUpdate = _records.Values.FirstOrDefault();
                return true;
            }
            if(!_records.ContainsKey(recordToUpdate.Name))
            {
                Console.WriteLine("No person is called " + recordToUpdate.Name + ".");
                return false;
            }
            else
            {
                _records[recordToUpdate.Name] = recordToUpdate;
                recordToUpdate.LastestUpdateDate = DateTime.Now;
                return true;
            }
        }
