...
namespace Hik.Communication.ScsServices.Communication.Messages
{
    ...
    [ProtoContract]
    internal class ScsRemoteExceptionSurrogate
    {
        [ProtoMember(1)]
        public string Message { get; set; }
        public static implicit operator ScsRemoteExceptionSurrogate(ScsRemoteException value)
        {
            if (value == null)
                return null;
            return new ScsRemoteExceptionSurrogate { Message = value.Message };
        }

        public static implicit operator ScsRemoteException(ScsRemoteExceptionSurrogate value)
        {
            if (value == null)
                return null;
            return new ScsRemoteException(value.Message);
        }
    }
}
