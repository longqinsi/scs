...
namespace Hik.Communication.Scs.Client
{
    ...
    internal abstract class ScsClientBase : IScsClient
    {
        ...
        private void PingTimer_Elapsed(object sender, EventArgs e)
        {
            ...
            try
            {
                #region modified by longqinsi 20140516
                //var lastMinute = DateTime.Now.AddMinutes(-1);
                var twentySecondsAgo = DateTime.Now.AddSeconds(-20);
                #endregion
                if (_communicationChannel.LastReceivedMessageTime > twentySecondsAgo || _communicationChannel.LastSentMessageTime > twentySecondsAgo)
                {
                    return;
                }
